...
public class BuildKymographs extends BuildSeries  
{
	...
	{	
		...
		for (int t = 0; t < exp.seqKymos.seq.getSizeT(); t++) {
			...
			futuresArray.add(processor.submit(new Runnable () {
				@Override
				public void run() {	
					Spot cap = exp.capillaries.spotsList.get(t_index);
	...
	{
		if (exp.capillaries.spotsList.size() < 1) {
	    ...
	    processor.setPriority(Processor.NORM_PRIORITY);
	    int ntasks =  exp.capillaries.spotsList.size(); //
		...
		for (long ii_ms = exp.kymoFirst_ms ; ii_ms <= exp.kymoLast_ms; ii_ms += exp.kymoBin_ms, iToColumn++) {
			...
			tasks.add(processor.submit(new Runnable () {
				@Override
				public void run() {	
					for (Spot capi: exp.capillaries.spotsList) 
...
//			seqData.setImage(0, 0, sourceImage); // add option??
	...
	private void analyzeImageWithCapillary(IcyBufferedImage sourceImage, Spot cap, int fromSourceImageIndex, int kymographColumn)
	{
	...
	private void exportCapillaryIntegerArrays_to_Kymograph(Experiment exp, Sequence seqKymo, final int sizeC)
	{
	    ...
	    processor.setThreadName("buildKymograph");
	    processor.setPriority(Processor.NORM_PRIORITY);
	    int nbcapillaries =  exp.capillaries.spotsList.size(); 
	    ArrayList<Future<?>> tasks = new ArrayList<Future<?>>( nbcapillaries);
		tasks.clear();
		 
		for (int icap = 0; icap < nbcapillaries; icap++) {
			final Spot cap = exp.capillaries.spotsList.get(icap);
	...
	private void exportOneCapillaryIntegerArray_to_Kymograph(Sequence seqKymo, int icap, Spot cap, IcyBufferedImage cap_Image, int sizeC)
	{
